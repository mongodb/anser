...
func (m *migrationBase) SaveMigrationEvent(ctx context.Context, mm *model.MigrationMetadata) error {
	...
	res, err := client.Database(ns.DB).Collection(ns.Collection).UpdateOne(ctx,
		birch.DC.Elements(birch.EC.String("_id", mm.ID)),
		mm, options.Update().SetUpsert(true))
	if err != nil {
		return errors.WithStack(err)
	}

	if res.UpsertedCount+res.ModifiedCount != 1 {
		return errors.New("migration event was not saved")
	}
...
func (m *migrationBase) GetMigrationEvents(ctx context.Context, q map[string]interface{}) MigrationMetadataIterator {
	...
	return &cursorMigrationMetadataIterator{cursor: cursor, catcher: grip.NewBasicCatcher()}
}
